...
# Define input files
READ1="$FASTQ_DIR/R1.fastq.gz"
READ2="$FASTQ_DIR/R2.fastq.gz"

# Define output files
OUTPUT_SAM="$RESULTS_DIR/R1R2.sam"

# Aligning the FastQC seq to Ref. genome (hg38)
"$BWA_BINARY" mem -t $(nproc) -R "@RG\tID:ctrl1\tSM:normal\tPL:ILLUMINA\tLB:lib1\tPU:unit1" "$GENOME_REF" "$READ1" "$READ2" > "$OUTPUT_SAM"
